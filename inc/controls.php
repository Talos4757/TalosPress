...
if ( ! class_exists( 'Generate_Customize_Width_Slider_Control' ) ) :
	class Generate_Customize_Width_Slider_Control extends WP_Customize_Control {
		...
		public function render_content() { ?>
			...
			<input name="<?php echo $this->id; ?>" type="text" <?php $this->link(); ?>
			       value="<?php echo $this->value(); ?>" style="display:none"/>
			<div class="slider"></div>
			<p class="description"><?php _e( 'Width', 'generate' );?> - <strong
					class="value"><?php echo $this->value(); ?></strong>px</p>
		<?php
		}
		...
		public function enqueue() {
			...
			wp_enqueue_script( 'generate_customizer', get_template_directory_uri() . '/js/customizer.js', array( 'jquery' ), GENERATE_VERSION, true );
	...
	class Generate_Upload_Control extends WP_Customize_Control {
		...
		public function render_content() {
			...
			<div class='generate-upload'>
				...
				<a class="button upload" data-title="<?php _e( 'Select Image', 'generate' ); ?>"
				   data-button="<?php _e( 'Use Image', 'generate' ); ?>"><?php _e( 'Upload', 'generate' ); ?></a>
				<a class="button remove"
				   <?php if (empty( $value )) { ?>style="display:none;"<?php } ?>><?php _e( 'Remove', 'generate' ); ?></a>
